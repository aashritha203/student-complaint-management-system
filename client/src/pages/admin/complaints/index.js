...
export default function AdminComplaintsPage() {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed loading admin complaints portfolio.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!stats && loading) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-semibold text-slate-550">Loading administrative complaints panel...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 font-sans">
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 leading-tight">Manage Complaints</h1>
